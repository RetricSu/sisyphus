...
export class Agent {
  ...
  constructor({
    saveMemory = true,
    pipeResponse,
    promptName = readSettings().prompt.selectedPromptName,
  }: AgentProp) {
    this.role = "assistant";
    this.messages = [];
    ...
    if (toolNames.includes("send_tweet") || toolNames.includes("reply_tweet")) {
      ...
      toolBoxes.push(
        ...twitterTools.filter((t) => toolNames.includes(t.fi.function.name)),
      );
    }
  ...
  startChromaServer() {
    ...
    return new Promise(
      (resolve: (val: string) => void, reject: (error: string) => void) => {
        const child = spawn(
          "chroma",
          ["run", "--path", settings.memory.database.folderPath],
          {
            env: { ...process.env },
          },
        );
  ...
  async call({
    ...
    if (requestMsg) {
      const message = new AMessage(
        this.memoId,
        requestMsg.role,
        requestMsg.content as string,
      );
