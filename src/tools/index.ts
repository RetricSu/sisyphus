...
export async function buildToolCallResponseCMessage(content: string) {
  ...
  switch (toolCall.name) {
    ...
    case AvailableToolName.callTerminalSimulator:
      functionResponse = toolExecutor.call_terminal_simulator(toolCall.parameters.command);
      break;

    case AvailableToolName.readWebpageContent:
      functionResponse = await toolExecutor.read_webpage_content(toolCall.parameters.url);
      break;
